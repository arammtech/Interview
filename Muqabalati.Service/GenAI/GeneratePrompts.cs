...
namespace Muqabalati.Service.GenAI
{
    public static class GeneratePrompts
    {
        ...
        {
            return $@"
يرجى تحليل البيانات التالية المستندة إلى مقابلة حقيقية مع مراعاة أن الإجابة يجب أن تكون منطقية وصحيحة، مع تجنب قبول إجابات غير دقيقة أو غامضة مثل ""هذا السؤال الأول"". وإرجاع النتائج بالتنسيق التالي:
...
<تقييم>
    ...
    4. <توصيات>
        <توصية>
            <نص>
                [نص التوصية المحددة لتحسين الأداء، بدون أي علامات مثل الاستفهام أو النقاط أو الفواصل أو الشرطات أو أي رموز أخرى]
            </نص>
            <مصدر>
                [رابط المصدر المستخدم في إعداد التوصية، أو قيمة فارغة إذا لم يوجد مصدر]
            </مصدر>
        </توصية>
        [قم بإضافة 3 توصيات على الأقل]
        [اجعل نص التوصية لا يتجاوز 100 كلمة]
        [يجب أن توفر على الأقل مصدرين موثوقين للتوصيات]
    </توصيات>
</تقييم>
...
3. **التنسيق المعتمد:**
   - النص يجب أن يلتزم بالشكل التالي فقط:
     ...
     <سؤال>
     1. [كلمة الربط]
     2. [السؤال الأصلي  لا يُسمح باستخدام علامات التنصيص مثل: ` أو ]
     3. [إعادة الصياغة مع أمثلة إذا لزم الأمر  لا يُسمح باستخدام علامات التنصيص مثل: ` أو ]
     4. [التوضيح مع أمثلة عملية إذا أمكن  لا يُسمح باستخدام علامات التنصيص مثل: ` أو ]
